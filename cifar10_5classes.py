...
def cifar_5_dataset(class1,class2,class3,class4,class5):
    import numpy as np
    import pandas as pd
...
#separation of images of class1,class2,class3,class4 and class5  so as to get total 25000  training images    
    j=0
    ro_train=[]
    for i in range(x_train.shape[0]):
        if y_train[i]==class1 or y_train[i]==class2 or y_train[i]==class3 or y_train[i]==class4 or y_train[i]==class5:
            ro_train.append(i)
    x_train1=np.array(x_train[ro_train])
    y_train1=np.array(y_train[ro_train])
    y_train2=[0 if i==class1 else 1 if i==class2 else 2 if i==class3 else 3 if i==class4 else 4 for i in y_train1 ]
#separation of images of class1,class2,class3,class4 and class5  so as to get total 5000  test images
    ro_test=[]
    for i in range(len(x_test)):
        if y_test[i]==class1 or y_test[i]==class2 or y_test[i]==class3 or y_test[i]==class4 or y_test[i]==class5:
            ro_test.append(i)
    x_test1=np.array(x_test[ro_test])
    y_test1=np.array(y_test[ro_test])
    y_test2=[0 if i==class1 else 1 if i==class2 else 2 if i==class3 else 3 if i==class4 else 4 for i in y_test1]


#converting the matrix of 25000x3072 to 25000x32x32x3 
    l=len(x_train1)
    x_train1=x_train1.reshape((l,3,32,32))
    x_train2=np.rollaxis(x_train1,1,4)
#converting the matrix of 5000x3072 to 5000x32x32x3 
    l1=len(x_test1)
